     ...
     LinearAlgebra::TpetraWrappers::Vector<double, MemorySpace::Default> &w)
{
  ...
  LinearAlgebra::TpetraWrappers::SparseMatrix<double, MemorySpace::Default> m(v.size(),
                                                        w.size(),
                                                        w.size());
...
main(int argc, char **argv)
{
  ...
  try
    {
      {
        LinearAlgebra::TpetraWrappers::Vector<double, MemorySpace::Default> v(complete_index_set(95),
                                                        MPI_COMM_SELF);
        LinearAlgebra::TpetraWrappers::Vector<double, MemorySpace::Default> w(complete_index_set(100),
                                                        MPI_COMM_SELF);
        test(v, w);
      }
    }
