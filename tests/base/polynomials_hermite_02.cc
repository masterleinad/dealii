...
// plot PolynomialsHermite using generate-complete_basis specifying support
// points
  ...
  for (unsigned int degree = 3; degree < 6; ++degree)
    {
      ...
      for (unsigned int i = 0; i < degree - 1; ++i)
        support_points.push_back(i * 1. / (degree - 2));

      const auto polynomials =
        PolynomialsHermite<double>::generate_complete_basis(support_points);
