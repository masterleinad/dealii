      ...
      if(cell->subdomain_id() == this_mpi_process)
        {
          ...
          const FEValues<dim> & fe_values
            = fe_values_hp.get_present_fe_values();
                      ...
                      if(i_group == fe_group && j_group == fe_group) // fe - fe
                        {
                          for(unsigned int q_point = 0; q_point < n_q_points;
                              ++q_point)
                            {
                              cell_stiffness_matrix(i, j)
                                += (fe_values[fe_extractor].gradient(i, q_point)
                                      * fe_values[fe_extractor].gradient(
                                          j, q_point)
                                      * 0.5
                                    + potential_values[q_point]
                                        * fe_values[fe_extractor].value(i,
                                                                        q_point)
                                        * fe_values[fe_extractor].value(
                                            j, q_point))
                                   * fe_values.JxW(q_point);
                              ...
                              && j_group == pou_group) // fe - pou
                        {
                          for(unsigned int q_point = 0; q_point < n_q_points;
                              ++q_point)
                            {
                              cell_stiffness_matrix(i, j)
                                += (fe_values[fe_extractor].gradient(i, q_point)
                                      * (fe_values[pou_extractor].gradient(
                                           j, q_point)
                                           * enrichment_values[q_point]
                                         + fe_values[pou_extractor].value(
                                             j, q_point)
                              ...
                              ++q_point)
                            {
                              cell_stiffness_matrix(i, j)
                                += ((fe_values[pou_extractor].gradient(i,
                                                                       q_point)
                                       * enrichment_values[q_point]
                                     + fe_values[pou_extractor].value(i,
                                                                      q_point)
                                         * enrichment_gradients[q_point])
                                      * fe_values[fe_extractor].gradient(
                                          j, q_point)
