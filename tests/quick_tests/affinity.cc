...
main()
{
  ...
  if(bits_set != 0 && bits_set != nprocs)
    {
      printf("Warning: sched_getaffinity() returns that we can only use %d out "
             "of %d CPUs.\n",
             bits_set,
             nprocs);
      return 2;
  ...
  if(env != -1 && nprocs != tbbprocs)
    {
      printf("Warning: number of threads is set to %d in environment using "
             "DEAL_II_NUM_THREADS.\n",
             env);
