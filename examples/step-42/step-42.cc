...
namespace Step42
{
  ...
  // Newton method, using Trilinos and p4est for parallel distributed
  // computing. To deal with hanging nodes makes life a bit more complicated
  // since we need another AffineConstraints now. We create a Newton method for
  // the active set method for the contact situation and to handle the nonlinear
  ...
  void PlasticityContactProblem<dim>::update_solution_and_constraints()
  {
              ...
              cell->face(face)->boundary_id() == 1)
            {
              ...
              for (unsigned int q_point = 0; q_point < n_face_q_points;
                   ++q_point)
                {
                  ...
                  // corresponding element to the correct value, and add the
                  // index to the IndexSet object that stores which degree of
    ...
    // those from hanging nodes into the AffineConstraints object that
    // already contains the active set. We finish the function by outputting the
    // total number of actively constrained degrees of freedom for which we sum
    // over the number of actively constrained degrees of freedom owned by each
  ...
  // particular in AffineConstraints::distribute_local_to_global(),
