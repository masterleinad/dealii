...
namespace Step64
{
    ...
    double                                                 *dst) const
  {
    ...
    fe_eval.read_dof_values(src); // 22
    fe_eval.evaluate(EvaluationFlags::values | EvaluationFlags::gradients); // 36
    fe_eval.apply_for_each_quad_point(
      HelmholtzOperatorQuad<dim, fe_degree>(gpu_data, coef, cell)); //54 without 81
    fe_eval.integrate(EvaluationFlags::values | EvaluationFlags::gradients);
    fe_eval.distribute_local_to_global(dst); //81, only this 73
  }
  ...
  try
    {
      ...
      HelmholtzProblem<3, 10> helmholtz_problem;
