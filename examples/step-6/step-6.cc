...
void Step6<dim>::setup_system()
{
  ...
  // we put all constraints on our function space in the
  // AffineConstraints. We can add constraints to the
  // AffineConstraints in either order: if two constraints conflict then
  // the constraint matrix either abort or throw an exception via the Assert
  // macro.
...
// AffineConstraints::distribute_local_to_global() that internally
// executes this loop while performing Gaussian elimination on rows and columns
