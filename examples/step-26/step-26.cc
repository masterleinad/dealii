...
namespace Step26
{
                                   ...
                                   const unsigned int component) const
  {
    ...
    /*    if (time > .28125 && time < .3125 && p(1) > .5 && p(0) > 0.75)
          return 1;
        if (time > .34375 && time < .375 && p(1) > .5 && p(0) > 0.5 && p(0) <
       .75) return 1; if (time > .40625 && time < .4375 && p(1) > .5 && p(0) >
       0.25 && p(0) < 0.5) return 1; if (time > .46875 && time < 0.5 && p(1) >
       .5 && p(0) > 0. && p(0) < 0.25) return 1;*/
                                      ...
                                      const double       time)
  {
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        if (time > 0.25 &&
            (cell->center()[1] > .75 && cell->center()[0] > 0.75))
        ...
        if (time > 0.375 &&
          ...
          cell->set_active_fe_index(0);
        if (time > 0.4375 &&
            (cell->center()[1] > .75 && cell->center()[0] > 0.))
          cell->set_active_fe_index(0);
      }
