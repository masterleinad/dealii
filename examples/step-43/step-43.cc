...
namespace Step43
{
  ...
  //
  // Unlike step-31, this step uses one more AffineConstraints object
  // called darcy_preconditioner_constraints. This constraint object is used
  // only for assembling the matrix for the Darcy preconditioner and includes
  // hanging node constraints as well as Dirichlet boundary value constraints
  // for the pressure variable. We need this because we are building a Laplace
  // matrix for the pressure as an approximation of the Schur complement) which
  // is only positive definite if boundary conditions are applied.
  //
  ...
  void TwoPhaseFlowProblem<dim>::assemble_darcy_preconditioner()
  {
    ...
    cell_iterator cell  = darcy_dof_handler.begin_active();
    cell_iterator endc  = darcy_dof_handler.end();
    cell_iterator saturation_cell = saturation_dof_handler.begin_active();
  ...
  // local_dof_indices. This is done by letting the AffineConstraints
  // class do the insertion of the cell matrix elements to the global matrix,
  // which already condenses the hanging node constraints.
  template <int dim>
  ...
  void TwoPhaseFlowProblem<dim>::assemble_saturation_rhs()
  {
    ...
    using cell_iterator      = typename DoFHandler<dim>::active_cell_iterator;
    cell_iterator cell       = saturation_dof_handler.begin_active();
    cell_iterator endc       = saturation_dof_handler.end();
    cell_iterator darcy_cell = darcy_dof_handler.begin_active();
