...
namespace Step57
{
  ...
  // complement preconditioner. We use one AffineConstraints for
  // Dirichlet boundary conditions at the initial step and a zero
  // AffineConstraints for the Newton is defined by 1/Re which has been
  ...
  // applied to our solution through AffineConstraints which is obtained
  // by using VectorTools::interpolate_boundary_values. The components of
  // boundary value functions are required to be chosen according to the finite
  // element space. Therefore we have to define the boundary value of pressure
  // even though we actually do not need it.
