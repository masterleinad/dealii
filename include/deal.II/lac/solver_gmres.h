...
public:
   ...
   */
  boost::signals2::connection
  connect_re_orthogonalize_slot(const std::function<void (int)> &slot);
...
protected:
  ...
  /**
   * Signal used to retrieve a notificatiothe when the vectors are re-orthogonalized.
   */
...
template <class VectorType>
boost::signals2::connection
SolverGMRES<VectorType>::connect_re_orthogonalize_slot
