...
namespace LinearAlgebra
{
  namespace internal
  {
    ...
    template <typename Number>
    struct read_write_vector_functions<Number, Host>
    {
      ...
      import(const std::shared_ptr<const ::dealii::Utilities::MPI::Partitioner>
             ...
             ::dealii::LinearAlgebra::ReadWriteVector<Number> &rw_vector)
      {
        distributed::Vector<Number, Host> tmp_vector(communication_pattern);
    ...
    template <typename Number>
    struct read_write_vector_functions<Number, CUDA>
    {
      using size_type = types::global_dof_index;
