...
namespace LinearAlgebra
{
  ...
  namespace TpetraWrappers
  {
    namespace internal
    {
      ...
      {
        Assert(&src != &dst,
               (typename SparseMatrix<double, MemorySpace>::ExcSourceEqualsDestination()));
        Assert(M.trilinos_matrix().isFillComplete(),
               (typename SparseMatrix<double, MemorySpace>::ExcMatrixNotCompressed()));

        if (mode == Teuchos::NO_TRANS)
          {
            Assert(src.trilinos_vector().getMap()->isSameAs(
                     *M.trilinos_matrix().getDomainMap()),
                   (typename SparseMatrix<double, MemorySpace>::ExcColMapMismatch()));
            Assert(dst.trilinos_vector().getMap()->isSameAs(
                     *M.trilinos_matrix().getRangeMap()),
                   (typename SparseMatrix<double, MemorySpace>::ExcDomainMapMismatch()));
          }
        else
          {
            Assert(dst.trilinos_vector().getMap()->isSameAs(
