  ...
  namespace distributed
  {
    namespace internal
    {
      ...
      struct la_parallel_vector_templates_functions
      {
        ...
        resize_val(const types::global_dof_index /*new_alloc_size*/,
                   ...
                   Number *& /*values_dev*/)
        {
          la_parallel_vector_functions<Number,
                                       MemorySpace>::memory_space_assert();
        }
        ...
        static void
        linfty_norm_local(const Number * /*values*/,
                          const Number * /*values_dev*/,
      ...
      template <typename Number>
      struct la_parallel_vector_templates_functions<Number, Host>
      {
               ...
               Number *)
        {
          ...
          ::dealii::LinearAlgebra::distributed::Vector<Number, Host> tmp_vector(
            communication_pattern);
      ...
      template <typename Number>
      struct la_parallel_vector_templates_functions<Number, CUDA>
      {
        using size_type = types::global_dof_index;
        ...
        resize_val(const types::global_dof_index new_alloc_size,
                   types::global_dof_index &     allocated_size,
                   std::unique_ptr<Number[], decltype(&free)> &,
                   Number *&values_dev)
        {
          static_assert(
          ...
          if (new_alloc_size > allocated_size)
            {
              Assert(((allocated_size > 0 && values_dev != nullptr) ||
                      values_dev == nullptr),
                     ExcInternalError());

              Utilities::CUDA::free(values_dev);
              Utilities::CUDA::malloc(values_dev, new_alloc_size);
               ...
               Number *values_dev)
        {
          ...
          ::dealii::LinearAlgebra::distributed::Vector<Number, CUDA> tmp_vector(
            communication_pattern);
          ...
          if (operation == VectorOperation::add)
            ::dealii::LinearAlgebra::CUDAWrappers::kernel::add_permutated<
              ...
              values_dev, tmp_vector.begin(), indices_dev, tmp_n_elements);
          else
        ...
        template <typename RealType>
        static void
        linfty_norm_local(const Number *,
                          const Number *     values_dev,
                          const unsigned int size,
                          RealType &         result)
        {
    ...
    Vector<Number, MemorySpace>::resize_val(const size_type new_alloc_size)
    {
      internal::la_parallel_vector_templates_functions<Number, MemorySpace>::
        resize_val(new_alloc_size, allocated_size, values, values_dev);
    ...
    Vector<Number, MemorySpace>::zero_out_ghosts() const
    {
      if (values != nullptr)
        std::fill_n(values.get() + partitioner->local_size(),
                    ...
                    Number());
#if defined(DEAL_II_WITH_MPI) && defined(__CUDACC__)
      if (values_dev != nullptr)
        {
          cudaError_t cuda_error_code =
            cudaMemset(values_dev + partitioner->local_size(),
                       0,
...
#  if defined(DEAL_II_WITH_CUDA) && defined(__CUDACC__)
      ...
      if (std::is_same<MemorySpace, CUDA>::value)
        {
          Number *new_val;
...
#  if defined(DEAL_II_WITH_CUDA) && defined(__CUDACC__)
      ...
      if (std::is_same<MemorySpace, CUDA>::value)
        {
          cudaError_t cuda_error_code =
            cudaMemcpy(values_dev + partitioner->local_size(),
                       values.get() + partitioner->local_size(),
                       partitioner->n_ghost_indices() * sizeof(Number),
                       cudaMemcpyHostToDevice);
          AssertCuda(cuda_error_code);
...
#  endif
      ...
      std::shared_ptr<const CommunicationPatternBase> communication_pattern)
    {
      ...
      if (std::is_same<MemorySpace, Host>::value)
        {
...
#  endif
      ...
      std::swap(values_dev, v.values_dev);
    ...
    Vector<Number, MemorySpace>::operator=(const Number s)
    {
      ...
      if (this_size > 0)
        {
          dealii::internal::VectorOperations::
            functions<Number, Number, MemorySpace>::set(
              thread_loop_partitioner, this_size, s, values.get(), values_dev);
        }
    ...
    Vector<Number, MemorySpace>::linfty_norm_local() const
    {
      ...
      internal::la_parallel_vector_templates_functions<Number, MemorySpace>::
        linfty_norm_local(values.get(), values_dev, local_size, max);
                                       ...
                                       const bool         across) const
    {
      ...
      if (std::is_same<MemorySpace, Host>::value)
        {
          std::copy(values.get(),
                    values.get() + allocated_size,
                    stored_elements.begin());
        }
      else
        {
          Utilities::CUDA::copy_to_host(values_dev, stored_elements);
        }
