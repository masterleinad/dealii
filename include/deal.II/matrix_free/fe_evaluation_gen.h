...
// Get FE Data info in a static manner. The FE object can provide this (and much
...
struct get_FEData
{
  ...
  static constexpr bool isIsotropic =
    true; // FE_Q in all directions is e.g. isotropic
...
struct get_FEData<FE_RaviartThomas<dim>, dim, dir, base_fe_degree, c>
{
  ...
  static constexpr int  fe_degree_for_component =
    ((dir == c) ? base_fe_degree + 1 : base_fe_degree);
  static constexpr int fe_degree =
    ((dir == c) ? base_fe_degree + 1 : base_fe_degree);
  static constexpr unsigned int dofs_per_cell =
    Utilities::fixed_int_power<base_fe_degree + 1, dim - 1>::value * dim *
    (base_fe_degree + 2);
};
...
struct get_FEData<FE_Q<dim>, dim, dir, base_fe_degree, c>
{
  ...
  static constexpr int          fe_degree               = base_fe_degree;
  static constexpr unsigned int dofs_per_cell =
    Utilities::fixed_int_power<base_fe_degree + 1, dim>::value *
    dim /*=n_components*/;
};
