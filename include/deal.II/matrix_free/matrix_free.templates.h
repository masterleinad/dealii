      ...
      for (unsigned int nq = 0; nq < n_quad; nq++)
        {
          ...
          shape_info(no, nq, 0, 0)
            .reinit(quad[nq][0], dof_handler[no]->get_fe());
        }
  }
          ...
          MultithreadInfo::n_threads() > 1)
        {
          ...
          task_info.use_partition_partition =
            (additional_data.tasks_parallel_scheme ==
                 AdditionalData::partition_partition ?
               true :
               false);
          task_info.use_coloring_only =
            (additional_data.tasks_parallel_scheme == AdditionalData::color ?
               true :
               false);
        }
      else
      ...
      for (unsigned int i = 0; i < dof_info.size(); ++i)
        {
          ...
          dof_info[i].n_components =
            dof_handler[i]->get_fe().element_multiplicity(0);
      ...
      for (unsigned int fe_no = 0;
        ...
        for (unsigned int nq = 0; nq < n_quad; nq++)
          for (unsigned int q_no = 0; q_no < quad[nq].size(); ++q_no)
            {
              shape_info(no, nq, fe_no, q_no)
                .reinit(quad[nq][q_no], dof_handler[no]->get_fe(fe_no));
            }
  }
      ...
      for (unsigned int i = 0; i < dof_info.size(); ++i)
        {
          ...
          dof_info[i].dimension = dim;
          dof_info[i].n_components =
            dof_handler[i]->get_fe(0).element_multiplicity(0);
          dof_info[i].dofs_per_cell.push_back(
  ...
  const std::vector<IndexSet> &                locally_owned_set)
{
      ...
      if (dof_handlers.active_dof_handler == DoFHandlers::hp)
        {
          ...
          for (unsigned int ind = 0; ind < hpdof->get_fe_collection().size();
               ++ind)
      ...
      for (unsigned int fe_index = 0; fe_index < fes.size(); ++fe_index)
        {
          ...
          Assert(
            ...
            ExcMessage(
              "MatrixFree currently only works for DoFHandler with one base element"));
          ...
          AssertDimension(
            shape_info(no, 0, fe_index, 0).lexicographic_numbering.size(),
      ...
      for (unsigned int no = 0; no < n_fe; ++no)
        {
          ...
          // ok, now we are requested to use a level in a MG DoFHandler
          ...
          else if (dof_handlers.active_dof_handler == DoFHandlers::hp)
            {
              ...
              dof_info[no].read_dof_indices(
                local_dof_indices,
                shape_info(no, 0, cell_it->active_fe_index(), 0)
  ...
  size_info.make_layout(n_active_cells,
  ...
  if (task_info.use_multithreading == true)
    {
      dof_info[0].compute_renumber_parallel(boundary_cells,
                                            size_info,
                                            renumbering);
  ...
  else
    {
      ...
      dof_info[0].compute_renumber_serial(boundary_cells,
                                          size_info,
                                          renumbering);
      if (dof_handlers.active_dof_handler == DoFHandlers::hp)
        dof_info[0].compute_renumber_hp_serial(size_info,
                                               renumbering,
                                               irregular_cells);
    }
...
#endif
  {
    ...
    AssertDimension(cell_level_index.size(),
                    size_info.n_macro_cells * vectorization_length);
  }
  ...
  for (unsigned int no = 0; no < n_fe; ++no)
    dof_info[no].reorder_cells(size_info,
                               renumbering,
...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    template <typename StreamType>
    void
    SizeInfo::print_memory_statistics(StreamType &out,
    ...
    inline void
    SizeInfo::make_layout(const unsigned int         n_active_cells_in,
                          ...
                          std::vector<unsigned int> &irregular_cells)
    {
      ...
      */
      ...
      unsigned int fillup_needed =
        (vectorization_length - n_boundary_cells % vectorization_length) %
        vectorization_length;
      ...
      Assert(n_boundary_cells % vectorization_length == 0 ||
               n_boundary_cells == n_active_cells,
             ExcInternalError());
      n_macro_cells =
      ...
      if (n_procs > 1)
        {
          const unsigned int n_macro_boundary_cells =
            (n_boundary_cells + vectorization_length - 1) /
            vectorization_length;
