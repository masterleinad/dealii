...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    {
      // Due to structure of FE which we use (RT and FE_Q), we want to store
      // data for max two dimensions only
      ...
      if (n_q_points_1d % 2 == 1 && n_dofs_1d % 2 == 1)
        {
          for (unsigned int i = 0; i < n_dofs_1d / 2; ++i)
            if (std::abs(get_first_array_element(
              ...
              return false;
          if (std::abs(get_first_array_element(
                         shape_values[(n_dofs_1d / 2) * n_q_points_1d +
                                      n_q_points_1d / 2]) -
                       1.) > zero_tol)
            return false;
        }
    ...
    ShapeInfo<Number>::lexicographic_renumber(
      const FiniteElement<dim> &fe_in,
      const unsigned int        base_element_number)
    {
            ...
            for (unsigned int comp = 0; comp < mul; ++comp)
              for (unsigned int i = 0; i < scalar_inv.size(); ++i)
                lexicographic[i * mul + comp + components_before] =
                  scalar_inv.size() * comp +
                  scalar_inv[i]; // TBD: Bug yet to be reported in dealii
    ...
    //			 <1st interior point for component 1, 1st interior points for
    //component 2...> 			 <2nd interior point for component 1, 2nd interior points
    //for component 2...>
    //			 .....
    // New ordering:
    ...
    template <int dim>
    void
    ShapeInfo<Number>::raviart_thomas_lexicographic_renumber(
      const FE_RaviartThomas<dim> *fe)
    {
      lexicographic_numbering.resize(fe->dofs_per_cell,
                                     numbers::invalid_unsigned_int);
      ...
      // Step1 : Reorder such that all (face+interior) points of a component are
      ...
      for (int i = 0; i < nip_per_comp; i++)
        for (int d = 0; d < dim; d++)
          lexicographic_numbering[n_per_comp * d + nfp_per_comp + i] =
            current++;
      ...
      // Step2: Reorder points of first component so that result has tensor
      // product structure. second component already
    ...
    template <int dim>
    void
    ShapeInfo<Number>::internal_reinit_scalar(
    ...
    template <int dim>
    void
    ShapeInfo<Number>::internal_reinit_vector(
      const Quadrature<1> &     quad,
      const FiniteElement<dim> &fe_in,
      const unsigned int        base_element_number)
    {
      ...
      std::array<unsigned int, 2>
        n_dofs_1d; // not more than 2 distinct values in case of RT
      ...
      fe_degree = fe->degree; // Note that for RT, this is max degree across all
      ...
      std::vector<std::vector<Polynomials::Polynomial<double>>>
                                      pols; // 1-D polynomials
      std::vector<PolynomialSpace<1>> polyspace;
      std::vector<FullMatrix<double>>
        tensor_inv_nodal; // tensor form of inverse nodal matrix
      ...
      if (FEName::FE_Q_TP == fe_name)
        {
          ...
          scalar_lexicographic =
            lexicographic_renumber(fe_in, base_element_number);
      ...
      else if (FEName::FE_RT == fe_name)
        {
          ...
          const FE_RaviartThomas<dim> *fe_rt =
            dynamic_cast<const FE_RaviartThomas<dim> *>(fe);
          raviart_thomas_lexicographic_renumber(fe_rt);

          pols =
            PolynomialsRaviartThomas<dim>::create_polynomials(fe_degree - 1);
          polyspace.reserve(base_values_count);
          for (int j = 0; j < base_values_count; j++)
            {
              polyspace.emplace_back(PolynomialSpace<1>(pols[j]));
              Assert(polyspace[j].n() == n_dofs_1d[j],
                     ExcMessage(
                       "Polynomial space size mismatch with expected dofs"));
            }

          // For n = 2, the last component helps to obtain the required tensor
          // product form for RT N matrices Reorder and extract last component
          // from tra(inverse nodal matrix)
          const FullMatrix<double> &inv_nodeM =
            fe_rt->get_inverse_node_matrix();
          ...
          FullMatrix<double> T(dofs_per_component_on_cell,
          ...
          FullMatrix<double> A(fe_degree, fe_degree),
            B(fe_degree + 1, fe_degree + 1);
              ...
              for (int j = 0; j < (fe_degree + 1); j++)
                {
                  ...
                  for (int x = 0; x < fe_degree; x++)
                    for (int y = 0; y < fe_degree; y++)
                      if (std::fabs(A(x, y)) >
                          10e-6) // Avoid division by ~zero, chosen by
                                 // experimentation
                        val = std::max(val,
                                       tempM(x, y) /
                                         A(x, y)); // too small values of tempM
                                                   // should not bias result
                  ...
                  for (unsigned int i = 0; i < n_dofs_1d[j]; ++i)
                    {
                      ...
                      this->base_shape_values[j][i * n_q_points_1d + q] =
                        tensor_inv_nodal[j](i, 0) * p_values[0];
                      for (int k = 1; k < psize; k++)
                        this->base_shape_values[j][i * n_q_points_1d + q] =
                          this->base_shape_values[j][i * n_q_points_1d + q] +
                          tensor_inv_nodal[j](i, k) * p_values[k];

                      this->base_shape_gradients[j][i * n_q_points_1d + q] =
                        tensor_inv_nodal[j](i, 0) * p_grads[0][0];
                      for (int k = 1; k < psize; k++)
                        this->base_shape_gradients[j][i * n_q_points_1d + q] =
                          this->base_shape_gradients[j][i * n_q_points_1d + q] +
                          tensor_inv_nodal[j](i, k) * p_grads[k][0];

                      // FIXME: Yet to be verified for hessians
                      this->base_shape_hessians[j][i * n_q_points_1d + q] =
                        tensor_inv_nodal[j](i, 0) * p_grad_grads[0][0][0];
                      for (int k = 1; k < psize; k++)
                        this->base_shape_hessians[j][i * n_q_points_1d + q] =
                          this->base_shape_hessians[j][i * n_q_points_1d + q] +
                          tensor_inv_nodal[j](i, k) * p_grad_grads[k][0][0];
                    }
                }
            }
          ...
          if (FEName::FE_Q_TP == fe_name)
            {
              ...
              std::vector<unsigned int> scalar_lexicographic_temp =
                temp_fe.get_poly_space_numbering_inverse();
                  ...
                  for (unsigned int q = 0; q < n_q_points_1d; ++q)
                    {
                      ...
                      this->base_shape_values[j][i * n_q_points_1d + q] =
                        temp_fe.shape_value(my_i, q_point);
                      this->base_shape_gradients[j][i * n_q_points_1d + q] =
                        temp_fe.shape_grad(my_i, q_point)[0];
                      this->base_shape_hessians[j][i * n_q_points_1d + q] =
                        temp_fe.shape_grad_grad(my_i, q_point)[0][0];
                    }
                }
            }
        }
      ...
      for (int c = 0; c < vector_n_components; c++)
        {
          for (int d = 0; d < dim; d++)
            {
              shape_gradients_vec[c][d] =
                this->base_shape_gradients[index_map[mindex(c, d)]].begin();
              shape_values_vec[c][d] =
                this->base_shape_values[index_map[mindex(c, d)]].begin();
              shape_hessians_vec[c][d] =
                this->base_shape_hessians[index_map[mindex(c, d)]].begin();
            }
        }
    }
