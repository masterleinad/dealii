...
namespace Functions
{
  ...
  public:
    ...
    void vector_gradient_list (const std::vector<Point<dim> > &,
  ...
  public:
     ...
     */
    void hessian_list (const std::vector<Point<dim> > &points,
  ...
  public:
    ...
    void value_list (const std::vector<Point<2> > &points,
                     std::vector<double>            &values,
                     const unsigned int              component = 0) const override;

    void vector_value_list (const std::vector<Point<2> > &points,
                            std::vector<Vector<double> > &values) const override;

    Tensor<1,2> gradient (const Point<2>     &p,
                          const unsigned int  component = 0) const override;

    void gradient_list (const std::vector<Point<2> > &points,
                        std::vector<Tensor<1,2> >    &gradients,
                        const unsigned int            component = 0) const override;

    void vector_gradient_list (const std::vector<Point<2> > &,
                               std::vector<std::vector<Tensor<1,2> > > &) const override;

    double laplacian (const Point<2>   &p,
                      const unsigned int  component = 0) const override;

    void laplacian_list (const std::vector<Point<2> > &points,
                         std::vector<double>          &values,
                         const unsigned int            component = 0) const override;
  };
