...
namespace internal::hp::DoFHandlerImplementation
{
  ...
  struct Implementation
  {
    ...
    template <int dim, int spacedim>
    static void
    ensure_absence_of_future_fe_indices(DoFHandler<dim, spacedim> &dof_handler)
    {
      (void)dof_handler;
              ...
              if (!cell->face(face)->user_flag_set())
                {
                  ...
                  if (cell->at_boundary(face) ||
                  ...
                  else
                    {
                      ...
                      face_dof_indices[next_free_face_slot + 1 +
      ...
      dealii::hp::DoFHandler<dim, spacedim> &dof_handler)
    {
      if (const dealii::parallel::shared::Triangulation<dim, spacedim> *tr =
              ...
              &dof_handler.get_triangulation()))
        {
          ...
          std::vector<unsigned int> active_fe_indices(tr->n_active_cells(), 0u);
          for (const auto &cell : dof_handler.active_cell_iterators())
            if (cell->is_locally_owned())
      ...
      else if (const dealii::parallel::distributed::Triangulation<dim, spacedim>
                   ...
                   &dof_handler.get_triangulation()))
        {
          ...
          // objects
          auto pack = [](const typename dealii::hp::DoFHandler<dim, spacedim>::
                    ...
                    fe_transfer->coarsened_cells_fe_index.end())
                  {
                    ...
                    for (unsigned int child_index = 0;
                         ...
                         ++child_index)
                      {
                        ...
                        fe_indices_children.insert(sibling->future_fe_index());
                      }
