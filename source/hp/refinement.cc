...
namespace hp::Refinement
{
  ...
  {
    std::vector<bool> p_flags(dof_handler.get_triangulation().n_active_cells(),
                              true);
    ...
    const ComparisonFunction<typename identity<Number>::type> &compare_coarsen)
  {
    ...
    const Number threshold_refine = min_criterion_refine +
                                    p_refine_fraction * (max_criterion_refine -
                                                         min_criterion_refine),
                 threshold_coarsen =
                   min_criterion_coarsen +
                   p_coarsen_fraction *
    ...
    const ComparisonFunction<typename identity<Number>::type> &compare_coarsen)
  {
    ...
    if (parallel_tria != nullptr &&
          ...
          &dof_handler.get_triangulation()) == nullptr)
      {
        ...
        const unsigned int target_index_refinement = static_cast<unsigned int>(
          std::floor(p_refine_fraction * n_global_flags_refinement));
        const unsigned int target_index_coarsening = static_cast<unsigned int>(
        ...
        if (n_p_refine_cells == 0)
        ...
        else
          {
            std::nth_element(indicators_refinement.begin(),
                             indicators_refinement.begin() + n_p_refine_cells -
                               1,
        ...
        if (n_p_coarsen_cells == 0)
        ...
        else
          {
            std::nth_element(indicators_coarsening.begin(),
                             indicators_coarsening.begin() + n_p_coarsen_cells -
                               1,
    ...
    p_adaptivity_from_absolute_threshold(dof_handler,
                                         ...
                                         std::cref(reference_compare_coarsen));
  }
  ...
  template <int dim, typename Number, int spacedim>
  void
  p_adaptivity_from_regularity(const hp::DoFHandler<dim, spacedim> &dof_handler,
