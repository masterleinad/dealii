...
namespace internal::DoFHandlerImplementation::Policy
{
  ...
  namespace
  {
    ...
    {
      typename DoFHandlerType::active_cell_iterator beginc = dof_handler
                                                               .begin_active(),
    ...
    update_all_level_cell_dof_indices_caches(const DoFHandlerType &dof_handler)
    {
      ...
      auto worker = [](const typename DoFHandlerType::level_cell_iterator &cell,
                       void *,
                       void *) {
    ...
    using DoFIdentities = std::vector<std::pair<unsigned int, unsigned int>>;
  ...
  struct Implementation
  {
      ...
      const hp::DoFHandler<dim, spacedim> &dof_handler)
    {
      ...
      for (unsigned int vertex_index = 0;
           ...
           ++vertex_index)
        if (dof_handler.get_triangulation().get_used_vertices()[vertex_index] ==
            true)
          {
            ...
            if (n_active_fe_indices > 1)
              {
                const std::set<unsigned int> fe_indices =
                  dealii::internal::DoFAccessorImplementation::Implementation::
                    get_active_vertex_fe_indices(dof_handler, vertex_index);
                                ...
                                for (unsigned int j = 0; j < dofs_per_line; ++j)
                                  {
                                    const types::global_dof_index
                                      master_dof_index =
                                        line->dof_index(j, dominating_fe_index);
                                    const types::global_dof_index
                                      slave_dof_index =
    ...
    template <int spacedim>
    static std::map<types::global_dof_index, types::global_dof_index>
    compute_quad_dof_identities(const hp::DoFHandler<3, spacedim> &dof_handler)
    {
      const int dim = 3;
                        ...
                        for (const auto &identity : identities)
                          {
                            const types::global_dof_index master_dof_index =
                              quad->dof_index(identity.first,
                                              most_dominating_fe_index);
                            const types::global_dof_index slave_dof_index =
                              quad->dof_index(identity.second, other_fe_index);
      ...
      const DoFHandlerType &)
    {
      ...
      for (const types::global_dof_index new_dof_index : new_dof_indices)
        {
          (void)new_dof_index;
          Assert(new_dof_index != enumeration_dof_index, ExcInternalError());
    ...
    template <int spacedim>
    static void
      merge_invalid_line_dofs_on_ghost_interfaces(hp::DoFHandler<1, spacedim> &)
    {}
    ...
    template <int dim, int spacedim>
    static void
    merge_invalid_quad_dofs_on_ghost_interfaces(hp::DoFHandler<dim, spacedim> &)
    {
      // this function should only be called for dim<3 where there are
      // no quad dof identies. for dim>=3, the specialization below should
      // take care of it
      ...
      if (dof_handler.get_fe().dofs_per_line > 0)
        {
          ...
          for (cell = dof_handler.begin(level); cell != endc; ++cell)
            if (cell->level_subdomain_id() != numbers::artificial_subdomain_id)
              for (const unsigned int line : GeometryInfo<2>::face_indices())
  ...
  ParallelShared<DoFHandlerType>::distribute_dofs() const
  {
                                                           ...
                                                           IndexSet(n_dofs));
    {
      ...
      while (start_index < n_dofs)
        {
          while ((end_index) < n_dofs && (subdomain_association[end_index] ==
    ...
    for (unsigned int lvl = 0; lvl < n_levels; ++lvl)
      {
        ...
        // finally, restore current level subdomain ids
        {
          ...
          number_caches.emplace_back(NumberCache(
            locally_owned_dofs_per_processor,
            this->dof_handler->get_triangulation().locally_owned_subdomain()));
        }
      }
  ...
  namespace
  {
        ...
        &neighbor_cell_list)
    {
      ...
      if (dealii_cell->user_flag_set() &&
        ...
        {
          neighbor_cell_list[dealii_cell->level_subdomain_id()].emplace_back(
            tree_index, dealii_cell->id().template to_binary<spacedim>());
        }
    }
      ...
      if (dealii_cell->id() == CellId(quadrant))
        {
          ...
          const_cast<typename DoFHandler<dim, spacedim>::level_cell_iterator &>(
            dealii_cell)
      ...
      for (unsigned int idx = 0; idx < level_ghost_owners.size(); ++idx)
        {
          ...
          Assert((len > 0 && !quadrants.empty()) ||
                 ...
                 ExcInternalError());
          std::vector<types::global_dof_index> receive_dof_numbers_and_indices(
            len);
          ...
          for (const auto &it : quadrants)
            {
              ...
              typename DoFHandlerType::level_cell_iterator cell(&tria,
                                                                0,
                                                                temp->index(),
                                                                &dof_handler);
...
#    endif
            return std_cxx17::optional<std::vector<types::global_dof_index>>();
          }
      };
...
#  endif
  ...
  ParallelDistributed<DoFHandlerType>::distribute_dofs() const
  {
    ...
    const int ierr = MPI_Exscan(DEAL_II_MPI_CONST_CAST(&n_locally_owned_dofs),
                                &my_shift,
                                1,
                                DEAL_II_DOF_INDEX_MPI_TYPE,
                                MPI_SUM,
