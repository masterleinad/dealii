    ...
    if (cm.n_constraints() > 0)
      {
        ...
        for (unsigned long long dofs_with_support_on_halo_cell :
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned long long &dof_indice : dof_indices)
            if (!dof_set.is_element(dof_indice))
              global_dof_indices.insert(dof_indice);
        }
      ...
      if (cell->is_ghost())
        {
          ...
          for (unsigned long long dof_indice : dof_indices)
            if (!dof_set.is_element(dof_indice))
              dofs_on_ghosts.push_back(dof_indice);
        }
        ...
        for (typename std::vector<
             ...
             ++it_cell)
          {
            ...
            for (unsigned long long &local_dof_indice : local_dof_indices)
              subdomain_halo_global_dof_indices.insert(local_dof_indice);
          }
