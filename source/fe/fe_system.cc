      ...
      while (true)
        {
          ...
          for (auto &base_identitie : base_identities)
            identities.emplace_back(base_identitie.first + dof_offset,
                                    base_identitie.second + dof_offset_other);
  ...
  for (unsigned int i = 0; i < base_elements.size(); ++i)
    {
      ...
      for (unsigned int r = 0; r < element_multiplicity; ++r)
        for (unsigned int c : base_table.second)
