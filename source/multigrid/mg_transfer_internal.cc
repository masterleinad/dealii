      ...
      if (tria)
        {
          ...
          // accordingly
          for (auto &dofpair : send_data_temp)
            {
          ...
          {
            for (unsigned int dest : neighbors)
              {
            ...
            for (unsigned int counter = 0; counter < neighbors.size();
                 ++counter)
              {
                ...
                for (auto &i : receive_buffer)
                  {
                    copy_indices_level_mine[i.level].emplace_back(
                      i.global_dof_index, i.level_dof_index);
                  }
              }
          }
...
#  endif
      ...
      std::less<std::pair<types::global_dof_index, types::global_dof_index>>
        compare;
      for (auto &copy_indice : copy_indices)
        std::sort(copy_indice.begin(), copy_indice.end(), compare);
      for (auto &level : copy_indices_level_mine)
        std::sort(level.begin(), level.end(), compare);
      for (auto &level : copy_indices_global_mine)
        std::sort(level.begin(), level.end(), compare);
    }
      ...
      if (ghosted_level_vector.size() == locally_owned.size())
        {
          ...
          for (auto &i : copy_indices_global_mine)
            i.second =
              locally_owned.n_elements() +
              ghosted_dofs.index_within_set(part->local_to_global(i.second));
        }
          ...
          for (cell = mg_dof.begin(level - 1); cell != endc; ++cell)
            {
              ...
              for (unsigned int c = 0;
                   ...
                   ++c)
                {
                  ...
                  for (unsigned long long local_dof_indice : local_dof_indices)
                    if (!owned_level_dofs.is_element(local_dof_indice))
                      ghosted_level_dofs.push_back(local_dof_indice);
              ...
              if (level == 1 && !cell_is_remote)
                {
                  ...
                  for (unsigned long long local_dof_indice : local_dof_indices)
                    if (!owned_level_dofs_l0.is_element(local_dof_indice))
                      ghosted_level_dofs_l0.push_back(local_dof_indice);
