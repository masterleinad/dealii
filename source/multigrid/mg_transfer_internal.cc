    ...
    const bool skip_interface_dofs)
  {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {
                ...
                bool global_mine = dof_handler.locally_owned_dofs().is_element(
    ...
    if (tria && Utilities::MPI::sum(send_data_temp.size(),
                                    tria->get_communicator()) > 0)
      {
        ...
        for (unsigned int level = 0; level < n_levels; ++level)
          {
            const IndexSet &is_local = dof_handler.locally_owned_mg_dofs(level);
    ...
    for (unsigned int level = n_levels - 1; level > 0; --level)
      {
        ...
        const parallel::TriangulationBase<dim, dim> *ptria =
          (dynamic_cast<const parallel::TriangulationBase<dim, dim> *>(&tria));
        const MPI_Comm communicator =
