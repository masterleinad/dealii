...
namespace
{
      ...
      &ghosted_level_vector)
  {
    ...
    for (unsigned int l = 0; l < mg_dof.get_triangulation().n_global_levels();
         ++l)
      {
        for (auto &i : my_copy_indices_level_mine[l])
          accessed_indices.push_back(i.first);
        std::vector<types::global_dof_index> accessed_level_indices;
        for (auto &i : my_copy_indices_global_mine[l])
          accessed_level_indices.push_back(i.second);
