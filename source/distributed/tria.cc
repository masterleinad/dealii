...
namespace parallel
{
  namespace distributed
  {
        ...
        &pack_callbacks_variable)
    {
      ...
      for (const auto &data_cell_fixed_it : packed_fixed_size_data)
        {
      ...
      if (variable_size_data_stored)
        {
          ...
          for (const auto &data_cell_variable_it : packed_variable_size_data)
            {
              int variable_data_size_on_cell = 0;

              for (const auto &data_variable_it : data_cell_variable_it)
      ...
      for (auto &data_cell_fixed_it : packed_fixed_size_data)
        {
          // Move every fraction of packed data into the buffer
          // reserved for this particular cell.
          for (auto &data_fixed_it : data_cell_fixed_it)
      ...
      if (variable_size_data_stored)
        {
          ...
          for (auto &data_cell_variable_it : packed_variable_size_data)
            {
              // Move every fraction of packed data into the buffer
              // reserved for this particular cell.
              for (auto &data_variable_it : data_cell_variable_it)
              ...
              if (send_to.size() > 0)
                {
                  ...
                  if (vertex_indices.size() > 0)
                    for (unsigned int subdomain : send_to)
                      {
