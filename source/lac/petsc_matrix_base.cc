...
#  endif
  ...
  MatrixBase::row_length(const size_type row) const
  {
    ...
    // function is called the same number of times from all processors, something
    // that is unreasonable. there should simply be a way in PETSc to query the
    // number of entries in a row bypassing the call to compress(), but I can't
    // find one
