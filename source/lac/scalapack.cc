  ...
  const std::pair<unsigned int, unsigned int> &submatrix_size) const
{
  ...
  Assert(offset_A.first <
           static_cast<unsigned int>(n_rows - submatrix_size.first + 1),
         ExcIndexRange(offset_A.first, 0, n_rows - submatrix_size.first + 1));
  Assert(offset_A.second <
           static_cast<unsigned int>(n_columns - submatrix_size.second + 1),
         ExcIndexRange(offset_A.second,
                       0,
                       n_columns - submatrix_size.second + 1));

  // range checking for matrix B
  Assert(offset_B.first <
           static_cast<unsigned int>(B.n_rows - submatrix_size.first + 1),
         ExcIndexRange(offset_B.first, 0, B.n_rows - submatrix_size.first + 1));
  Assert(offset_B.second <
           static_cast<unsigned int>(B.n_columns - submatrix_size.second + 1),
         ExcIndexRange(offset_B.second,
                       0,
                       B.n_columns - submatrix_size.second + 1));
...
namespace internal
{
  namespace
  {
    ...
    create_HDF5_state_enum_id(hid_t &state_enum_id)
    {
      ...
      status =
        H5Tenum_insert(state_enum_id, "lu", reinterpret_cast<int *>(&val));
      AssertThrow(status >= 0, ExcInternalError());
      val = LAPACKSupport::State::matrix;
      status =
        H5Tenum_insert(state_enum_id, "matrix", reinterpret_cast<int *>(&val));
      AssertThrow(status >= 0, ExcInternalError());
      val = LAPACKSupport::State::svd;
      status =
        H5Tenum_insert(state_enum_id, "svd", reinterpret_cast<int *>(&val));
      AssertThrow(status >= 0, ExcInternalError());
    ...
    create_HDF5_property_enum_id(hid_t &property_enum_id)
    {
      ...
      herr_t                  status = H5Tenum_insert(property_enum_id,
                                     "diagonal",
                                     reinterpret_cast<int *>(&prop));
      AssertThrow(status >= 0, ExcInternalError());
      prop   = LAPACKSupport::Property::general;
      status = H5Tenum_insert(property_enum_id,
                              "general",
                              reinterpret_cast<int *>(&prop));
      AssertThrow(status >= 0, ExcInternalError());
...
#  else
   ...
   */
  const int NB =
    std::max<int>(std::ceil(static_cast<double>(n_columns) / n_mpi_processes),
                  column_block_size);
