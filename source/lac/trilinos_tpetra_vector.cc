...
#    ifdef DEAL_II_HAVE_CXX20
    static_assert(concepts::is_vector_space_vector<Vector<float, MemorySpace::Host>>);
    static_assert(concepts::is_vector_space_vector<Vector<float, MemorySpace::Default>>);
#    endif
    ...
    Vector<float, MemorySpace::Host>::operator=<float>(const dealii::Vector<float> &);
    template Vector<float, MemorySpace::Default> &
    Vector<float, MemorySpace::Default>::operator=<float>(const dealii::Vector<float> &);
	    namespace internal
    {
...
#    ifdef DEAL_II_HAVE_CXX20
    static_assert(concepts::is_vector_space_vector<Vector<double, MemorySpace::Host>>);
        static_assert(concepts::is_vector_space_vector<Vector<double, MemorySpace::Default>>);
#    endif
    ...
    Vector<double, MemorySpace::Host>::operator=<double>(const dealii::Vector<double> &);
    template Vector<double, MemorySpace::Default> &
    Vector<double, MemorySpace::Default>::operator=<double>(const dealii::Vector<double> &);
    namespace internal
    {
...
#      endif
    template class Vector<std::complex<double>, MemorySpace::Host>;
     template class Vector<std::complex<double>, MemorySpace::Defaukt>;
    ...
    namespace internal
    {
      template class VectorReference<std::complex<double>, MemorySpace::Host>;
            template class VectorReference<std::complex<double>, MemorySpace::Default>;
    }
#    endif
#  endif
  } // namespace TpetraWrappers
