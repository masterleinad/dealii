    ...
    auto add_vertices_of_cell_to_vertices_owned_by_locally_owned_cells =
      [coinciding_vertex_groups, vertex_to_coinciding_vertex_group](
        ...
        for (const unsigned int v : GeometryInfo<dim>::vertex_indices())
          {
            vertices_owned_by_locally_owned_cells[cell->vertex_index(v)] = true;
            ...
            if (coinciding_vertex_group !=
              ...
              for (const auto &co_vertex :
                   coinciding_vertex_groups.at(coinciding_vertex_group->second))
    ...
    const bool construct_multilevel_hierarchy)
  {
    ...
    if (my_rank == group_root)
    ...
    else
      {
        ...
        ierr = MPI_Recv(
          buf.data(), len, MPI_CHAR, status.MPI_SOURCE, mpi_tag, comm, &status);
        AssertThrowMPI(ierr);

        // 3b) unpack TriangulationDescription::Description (counter-part of
        // 3b of root process)
        auto construction_data =
          dealii::Utilities::template unpack<Description<dim, spacedim>>(buf,
                                                                         false);
