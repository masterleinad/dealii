      ...
      if (i.second.n_elements() > 0)
        {
          ...
          import_indices_chunks_by_rank_data.push_back(
            import_indices_chunks_by_rank_data.back() + i.second.n_intervals());
        }
...
#  ifdef DEBUG
    ...
    int       flag = 0;
    const int ierr =
      MPI_Testall(requests.size(), requests.data(), &flag, MPI_STATUSES_IGNORE);
    AssertThrowMPI(ierr);
    Assert(flag == 1,
           ExcMessage(
...
#  endif
    ...
    if (larger_ghost_index_set.size() == 0)
    ...
    else
      {
        ...
        Assert(
          (larger_ghost_index_set & locally_owned_range_data).n_elements() == 0,
  ...
  Partitioner::memory_consumption() const
  {
    ...
    memory +=
      MemoryConsumption::memory_consumption(import_indices_chunks_by_rank_data);
