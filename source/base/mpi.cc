...
#  endif
                                ...
                                const IndexSet::size_type &local_size)
  {
    ...
    const std::vector<IndexSet::size_type> sizes = all_gather(comm, local_size);
...
#  endif
              ...
              const MPI_Comm &               mpi_communicator)
  {
    ...
    MPI_Op op;
    int ierr = MPI_Op_create(reinterpret_cast<MPI_User_function *>(&max_reduce),
                             true,
                             &op);
    AssertThrowMPI(ierr);
...
#  endif
  ...
  MPI_InitFinalize::~MPI_InitFinalize()
  {
    ...
    GrowingVectorMemory<
      LinearAlgebra::distributed::BlockVector<float>>::release_unused_memory();
