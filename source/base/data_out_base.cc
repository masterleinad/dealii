...
namespace DataOutBase
{
  ...
  {
    return "none|dx|ucd|gnuplot|povray|eps|gmv|tecplot|tecplot_binary|vtk|vtu|"
           "hdf5|svg|deal.II intermediate";
  }
    ...
    std::ostream&       out)
  {
    ...
    // preamble
    {
      out << "# This file was generated by the deal.II library." << '\n'
          ...
          << "# Time =  " << Utilities::System::get_time() << '\n'
          << "#" << '\n'
          << "# For a description of the Tecplot format see the Tecplot "
             "documentation."
          << '\n'
          << "#" << '\n';
...
#  endif
    ...
    std::ostream&   out)
  {
    ...
    // http://www.visitusers.org/index.php?title=Time_and_Cycle_in_VTK_files
    {
      ...
      if(flags.cycle != std::numeric_limits<unsigned int>::min())
        {
          out << "<DataArray type=\"Float32\" Name=\"CYCLE\" "
                 "NumberOfTuples=\"1\" format=\"ascii\">"
              << flags.cycle << "</DataArray>\n";
        }
      if(flags.time != std::numeric_limits<double>::min())
        {
          out << "<DataArray type=\"Float32\" Name=\"TIME\" "
                 "NumberOfTuples=\"1\" format=\"ascii\">"
                     ...
                     const std::vector<std::vector<std::string>>& piece_names)
  {
    ...
    for(std::vector<std::vector<std::string>>::const_iterator domain
        ...
        ++domain)
      {
        Assert(
          domain->size() == nblocks,
          ExcMessage("piece_names should be a vector of equal sized "
                     "vectors.")) for(std::vector<std::string>::const_iterator
                                        subdomain
                                      = domain->begin();
        ...
        for(unsigned int index = 0; index < 5; index++)
          {
            out << "  <text x=\""
                ...
                << "\""
                << " style=\"text-anchor:start; font-size:80; "
