...
// which does the rotated fill performed below and writes the data into the right
// data structures. The same function would be used by Nedelec polynomials.
  ...
  for (unsigned int d = 0; d < dim; ++d)
    {
      ...
      for (unsigned int i = 0; i < p_grads.size(); ++i)
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          grads[i + d * n_sub][d][(d1 + d) % dim] =
            p_grads[tensor_pols_mapping_inv[d][i]][d1];
        ...
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            for (unsigned int d3 = 0; d3 < dim; ++d3)
              third_derivatives[i + d * n_sub][d][(d1 + d) %
                                                  dim][(d2 + d) %
                                                       dim][(d3 + d) % dim] =
          ...
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            for (unsigned int d3 = 0; d3 < dim; ++d3)
              for (unsigned int d4 = 0; d4 < dim; ++d4)
                fourth_derivatives
                  [i + d * n_sub][d][(d1 + d) % dim][(d2 + d) % dim]
                  [(d3 + d) % dim][(d4 + d) % dim] =
                    p_fourth_derivatives[tensor_pols_mapping_inv[d][i]][d1][d2]
                                        [d3][d4];
    }
}
...
PolynomialsRaviartThomas<dim>::create_poly_mapping()
{
  ...
  // Actual indices are locations corresponding to transposed matrix of
  // calculated indices
  const unsigned int ny = my_degree + 2,
                     nx = my_degree + 1; // no of dofs in each direction
  if ((nx * ny) != n_sub)
