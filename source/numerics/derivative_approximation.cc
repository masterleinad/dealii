...
namespace DerivativeApproximation::internal
{
  /**
  ...
  SecondDerivative<2>::derivative_norm(const Derivative &d)
  {
    ...
    const double eigenvalues[2] = {0.5 *
                                     (d[0][0] + d[1][1] + std::sqrt(radicand)),
                                   0.5 *
                                     (d[0][0] + d[1][1] - std::sqrt(radicand))};
...
namespace DerivativeApproximation::internal::Assembler
{
  struct Scratch
  {
    Scratch() = default;
    ...
    WorkStream::run(
      ...
      [&mapping, &dof_handler, &solution, component](
        ...
        approximate<DerivativeDescription,
                    ...
                    DoFHandlerType,
                    InputVector,
                    spacedim>(cell, mapping, dof_handler, solution, component);
      },
